    ...
    if (
      ...
      if (nr_givens > difficulty) {
        ...
        for (i = 0; i < nr_givens - difficulty; ++i) {
          var target = parseInt(givens_idxs[i], 10);
          board = board.substr(0, target) +
            sudoku.BLANK_CHAR +
...
sudoku.validate_board = function(board) {
  ...
  if (board.length !== NR_SQUARES) {
    return 'Invalid board size. Board must be exactly ' +
      NR_SQUARES +
      ' squares.';
  }
  ...
  for (var i in board) {
    if (
      !sudoku._in(board[i], sudoku.DIGITS) && board[i] !== sudoku.BLANK_CHAR
    ) {
      return 'Invalid board character encountered at index ' +
        i +
        ': ' +
        board[i];
    }
